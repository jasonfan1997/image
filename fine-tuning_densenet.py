...
parser.add_argument('--mode', '-m', default=1, type=int, help='how many layers to fine tune')
...
val_dir = '../data/scene_classification/scene_validation_images_20170908'
data_base = '../data/'+name
aug_base = '../data/aug/'+name
#test_dir = '/home/bumsoo/Data/test/FINAL_TEST'
train_dir = '../data/scene_classification/scene_train_images_20170904'
...
data_dir = aug_base
dataset_dir = data_base.split("/")[-1] + os.sep
print("| Preparing model trained on %s dataset..." %(data_base.split("/")[-1]))
dsets = {
...
if (args.testOnly):
    ...
    top1 = AverageMeter()
    top5 = AverageMeter()
    testsets = datasets.ImageFolder(val_dir, data_transforms['val'])
    ...
    print("\n[Phase 3 : Inference on %s]" %val_dir)
    ...
    print("| Test Result: top1_acc = " +str(acc) +" top3_acc = " + str(acc3))
                ...
                if (phase == 'train'):
                    ...
                    sys.stdout.write('| Epoch [%2d/%2d] Iter [%3d/%3d]\t\tLoss %.4f\tAcc %.2f%%'
                            %(epoch+1, num_epochs, batch_idx+1,
                                (len(dsets[phase])//batch_size)+1, loss.data[0], 100.*running_corrects/tot))
                    sys.stdout.flush()
                    sys.stdout.write('\r')
...
if(args.resetClassifier):
    ...
    if(args.addlayer):
        print('| Add features of size %d' %feature_size)
        num_ftrs = model_ft.fc.in_features
        feature_model = list(model_ft.fc.children())
        feature_model.append(nn.Linear(num_ftrs, feature_size))
        feature_model.append(nn.BatchNorm1d(feature_size))
        feature_model.append(nn.ReLU(inplace=True))
        feature_model.append(nn.Linear(feature_size, len(dset_classes)))
